...
export default function LocationsPage() {
  return <CrudManager title="Property Locations" subtitle="These appear as location options when adding properties" apiBase="/api/admin/locations" icon={MapPin} />;
}

function CrudManager({ title, subtitle, apiBase, icon: Icon }) {
  ...
  useEffect(() => {
    apiFetch(apiBase).then((r) => setItems(r.data || [])).catch(() => toast.error('Failed to load'))
  ...
  const handleAdd = async () => {
    ...
    setAdding(true);
    try {
      const res = await apiFetch(apiBase, { method: 'POST', body: JSON.stringify({ name: newName.trim() }) });
      setItems((prev) => [...prev, res.data || { name: newName.trim(), _id: Date.now() }]);
      setNewName(''); toast.success('Added successfully');
    } catch (err) { toast.error(err.message || 'Failed to add'); }
    finally { setAdding(false); }
  };

  const startEdit = (item) => { setEditId(item._id); setEditName(item.name); };
  ...
  const handleUpdate = async (id) => {
    ...
    setUpdating(true);
    try {
      await apiFetch(`${apiBase}/${id}`, { method: 'PUT', body: JSON.stringify({ name: editName.trim() }) });
      setItems((prev) => prev.map((item) => item._id === id ? { ...item, name: editName.trim() } : item));
      cancelEdit(); toast.success('Updated');
  ...
  const handleDelete = async () => {
    try {
      await apiFetch(`${apiBase}/${deleteModal.id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <div className="bg-white rounded-lg p-5" style={{ border: '1px solid #E2E8F0' }}>
        <h3 className="font-semibold text-sm mb-3" style={{ color: '#0F172A' }}>Add New</h3>
        <div className="flex gap-2">
          <input value={newName} onChange={(e) => setNewName(e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleAdd()}
            placeholder={`Enter ${title.toLowerCase().replace('property ', '')} name…`}
      ...
      <div className="bg-white rounded-lg overflow-hidden" style={{ border: '1px solid #E2E8F0' }}>
        {loading
          ? Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="px-5 py-3.5 flex items-center gap-3" style={{ borderBottom: '1px solid #F1F5F9' }}>
                <div className="h-4 bg-slate-100 rounded animate-pulse w-40" />
              </div>
            ))
          : items.length === 0
          ? <p className="text-center py-12 text-sm" style={{ color: '#94A3B8' }}>No items yet. Add one above.</p>
          : items.map((item, idx) => (
                ...
                {editId === item._id ? (
                  <>
                    <button onClick={() => handleUpdate(item._id)} disabled={updating}
                      className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-green-50 transition-colors" style={{ color: '#22C55E' }}>
                      <Check className="w-4 h-4" />
                    </button>
                    <button onClick={cancelEdit}
                      className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-slate-100 transition-colors" style={{ color: '#94A3B8' }}>
                      <X className="w-4 h-4" />
                    </button>
                  </>
                ) : (
                  <>
                    <button onClick={() => startEdit(item)}
                      className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-slate-100 transition-colors" style={{ color: '#64748B' }}>
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => setDeleteModal({ open: true, id: item._id, name: item.name })}
                      className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-red-50 transition-colors" style={{ color: '#EF4444' }}>
      ...
      <ConfirmModal isOpen={deleteModal.open} title={`Delete "${deleteModal.name}"?`}
        message="This will remove this location from the list. Existing properties using it won't be affected."
